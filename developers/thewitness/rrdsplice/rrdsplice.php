...
foreach($parms as $parameter) {
	...
	case "--oldrrd":
		...
		if (!file_exists($oldrrd)) {
			echo "FATAL: File '$oldrrd' does not exist.\n";
			exit(-9);
		}

		if (!is_writable($oldrrd)) {
			echo "FATAL: File '$oldrrd' is not writable by this account.\n";
			exit(-8);
		}
	...
	case "--newrrd":
		...
		if (!file_exists($newrrd)) {
			echo "FATAL: File '$newrrd' does not exist.\n";
			exit(-9);
		}

		if (!is_writable($newrrd)) {
			echo "FATAL: File '$newrrd' is not writable by this account.\n";
			exit(-8);
		}
...
if ($oldrrd == "") {
	echo "FATAL: You must specify a old RRDfile!\n\n";
	display_help();
	exit(-2);
}
		...
		if (isset($new_rrd['rra'])) {
			foreach($new_rrd['rra'] as $rra_num => $rra) {
				...
				if (isset($rra['database'])) {
					...
					echo "FATAL: RRA database is Invalid\n";
				}
			}
		}else{
			echo "FATAL: One of RRA's is Invalid\n";
		}
	}else{
		echo "FATAL: One of RRD's is Invalid\n";
	}
}
...
function createRRDFileFromXML($xmlfile, $rrdfile) {
	/* execute the dump command */
	echo "NOTE: Re-Importing '$xmlfile' to '$rrdfile'\n";
	$response = shell_exec("rrdtool restore -f -r $xmlfile $rrdfile");
	if (strlen($response)) echo "$response\n";
}
...
function backupRRDFile($rrdfile) {
	...
	echo "NOTE: Backing Up '$rrdfile' to '" . $backupdir . "/" .  $newfile . "'\n";
...
function display_help () {
	...
	echo "Cacti RRDfile Splicer $version, Copyright 2011, The Cacti Group, Inc.\n\n";
	echo "Usage:\n";
	echo "rrdsplice.php --oldrrd=file --newrrd=file [--finrrd=file]\n";
	echo "                 [--debug] [--dryrun] [-h|--help|-v|-V|--version]\n\n";

	echo "The Old and New input parameters are mandatory.  If the finrrd option is\n";
	echo "not specified, it will be the newrrd plus a timestamp\n\n";

	echo "--oldrrd=file    - The old RRDfile that contains old data.\n";
	echo "--newrrd=file    - The new RRDfile that contains more recent data.\n";
	echo "--finrrd=file    - The final RRDfile that contains data from both rrdfiles.\n\n";

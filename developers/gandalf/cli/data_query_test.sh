...
# assume, that a host using SNMP version 2 is present (e.g. localhost changed to SNMP V2c)
...
php -q data_query_add.php --device-id=1 --data-query-id=1 --reindex-method=uptime
...
php -q data_query_add.php --device-id=1 --data-query-id=6 --reindex-method=uptime

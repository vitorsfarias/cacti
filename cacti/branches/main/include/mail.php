...
class Mailer {
	...
	function send($body, $headers = array())
	{
		...
		else
		{
			...
			if ($this->Message['mixed'])
			{
				$this->Message['body'] .= 'This is a multi-part message in MIME format' . self::EOL . self::EOL;
				$this->Message['body'] .= 'Content-Type: multipart/mixed; boundary=\'' . $boundary_mixed  . '\'' . self::EOL . self::EOL;
				$this->Message['body'] .= '--' . $boundary_mixed . self::EOL;
			}
			
			/* Open MIME related section */
			if ($this->Message['related'])
			{
				$this->Message['body'] .= 'Content-Type: multipart/related; boundary=\'' . $boundary_related  . '\'' . self::EOL . self::EOL;
				$this->Message['body'] .= '--' . $boundary_related . self::EOL;
			}
			
			/* Open MIME alternate section */
			if ($this->Message['alternate'])
			{
				$this->Message['body'] .= 'Content-Type: multipart/alternative; boundary=\'' . $boundary_alternate  . '\'' . self::EOL . self::EOL;
				$this->Message['body'] .= '--' . $boundary_alternate . self::EOL;
			}
			...
			if (! empty($body_text))
			{
				...
				$this->Message['body'] .= 'Content-Type: text/plain; charset=\'' . $this->Config['Mail']['CharSet'] . '\'' . self::EOL;
				$this->Message['body'] .= 'Content-Transfer-Encoding: 8-bit' . self::EOL;
				$this->Message['body'] .= '\n' . $body_text . self::EOL . self::EOL;
			}
			...
			if (! empty($body_html))
			{
				...
				$this->Message['body'] .= 'Content-Type: text/html; charset=\'' . $this->Config['Mail']['CharSet'] . '\'' . self::EOL;
				$this->Message['body'] .= 'Content-Transfer-Encoding: 8-bit'. self::EOL . self::EOL;
				$this->Message['body'] .= $body_html . self::EOL . self::EOL;
			}
			...
			if ($attachment_inline > 0)
			{
				foreach ($this->Message['attachments'] as $attachment)
				{
					...
					{
						$this->Message['body'] .= '--' . $boundary_attachment . self::EOL;
						/* we MUST have a CID for each INLINE attachment
						   this is the reference used in the text body
						*/
						$this->Message['body'] .= 'Content-ID: <' . $attachment['contentid'] . '>' . self::EOL;
						$this->Message['body'] .= 'Content-Type: ' . $attachment['contenttype'] . self::EOL;
						$this->Message['body'] .= 'Content-Transfer-Encoding: ' . $attachment['encoding'] . self::EOL;
						...
						$this->Message['body'] .= 'Content-Disposition: ' . $attachment['contentdisposition'] . $filename . self::EOL;
						$this->Message['body'] .= '\n' . $attachment['encoded_data'] . self::EOL;
					}
					else
					{
				...
				foreach ($this->Message['attachments'] as $attachment)
				{
					if ($attachment['contentdisposition'] != 'inline')
					{
						...
						*/
						...
						$this->Message['body'] .= 'Content-Type: ' . $attachment['contenttype'] . self::EOL;
						$this->Message['body'] .= 'Content-Transfer-Encoding: ' . $attachment['encoding'] . self::EOL;
						$this->Message['body'] .= 'Content-Disposition: ' . $attachment['contentdisposition'] . '; filename=\'' . $attachment['filename'] . '\'' . self::EOL;
			...
			if ($this->Message['related'])
			{
				$this->Message['body'] .= '--' . $boundary_related . '--' . self::EOL . self::EOL;
			}
	...
	function attach($data, $filename, $contenttype = 'application/octet', $contentdisposition = 'attachment', $contentid = '')
	{
		...
		/* Encode the data */
		$struc['encoded_data'] = chunk_split(base64_encode($data), 76, self::EOL);
	...
	function _send_mail()
	{
			...
			foreach ($header_order as $key)
			{
				if (($key != 'To') && ($key != 'Subject'))
				{
					if (isset($this->Message['headers'][$key]))
					{
						...
						{
							$headers .= $key . ': ' . implode(', ',$this->Message['headers'][$key]) . self::EOL;
						}
						else
						{
							$headers .= $key . ': ' . $this->Message['headers'][$key] . self::EOL;
						}
					}
				}
			}
		...
		elseif ($this->Config['Mail']['Type'] == 'SMTP')
		{
			...
			fputs($smtp_sock, 'HELO ' . $this->Config['Hostname'] . self::EOL);
			$smtp_response = fgets($smtp_sock, 4096);
			...
			{
				fputs($smtp_sock, 'AUTH LOGIN' . self::EOL);
				$smtp_response = fgets($smtp_sock, 4096);
				...
				fputs($smtp_sock, base64_encode($this->Config['Mail']['SMTP_Username']) . self::EOL);
				$smtp_response = fgets($smtp_sock, 4096);
				if (substr($smtp_response, 0, 3) != '334')
				{
				...
				fputs($smtp_sock, base64_encode($this->Config['Mail']['SMTP_Password']) . self::EOL);
				$smtp_response = fgets($smtp_sock, 4096);
			...
			/* Send mail from */
			fputs($smtp_sock, 'MAIL FROM: <' . $from . '>' . self::EOL);
			$smtp_response = fgets($smtp_sock, 4096);
			if (substr($smtp_response, 0, 3) != '250')
			{
			...
			foreach ($to as $item)
			{
				fputs($smtp_sock, 'RCPT TO: <' . $item . '>' . self::EOL);
				$smtp_response = fgets($smtp_sock, 4096);
				if (substr($smtp_response, 0, 3) != '250')
				{
			...
			fputs($smtp_sock, $headers . self::EOL . $this->Message['body'] . self::EOL . '.' . self::EOL);
			$smtp_response = fgets($smtp_sock,4096);
			if (substr($smtp_response, 0, 3) != '250')
			{
		...
		elseif ($this->Config['Mail']['Type'] == 'DirectInject')
		{
			...
			fwrite($pipes[0], $headers . self::EOL);
			fwrite($pipes[0], $this->Message['body'] . self::EOL);
			fclose($pipes[0]);

...
function classical_export($cacti_root_path, $cacti_export_path) {
	...
	$stats = __("Export Date: %s", __date(date_time_format())) . "<br>"; 
	$stats.= __("Total Graphs: %s", sizeof($graphs));
...
function build_html_file($leaf, $type = "", $array_data = array(), $snmp_index = "") {
	...
	case "tree":
		...
		break;
	case "leaf":
		$sql_join = "LEFT JOIN user_auth_perms ON (graph_templates_graph.local_graph_id=user_auth_perms.item_id AND user_auth_perms.type=" . PERM_GRAPHS . " AND user_auth_perms.user_id=" . read_config_option("export_user_id") . ")";
	...
	switch($type) {
	case "index":
		fwrite($fp, "<strong>" . __("Graphs Last Updated on:") . "</strong></td></tr>" .
				"<tr bgcolor='#a9b7cb'>" .
					"<td colspan='3' class='textHeaderDark'>" . 
						__("Export Date: %s", $array_data["timestamp"]) . "<br>" .
						__("Total Graphs: %s", $array_data["total_graphs_created"]) .
					"</td>" .
				"</tr><tr>");
		break;
	case "tree":
...
function export_tree_graphs_and_graph_html($path, $tree_id) {
	...
	foreach($graphs as $graph)  {
		...
		if (read_config_option("export_tree_isolation") == CHECKED) {
	...
	if (!isset($config["config_options_array"]["tree_array"]) || ($force_refresh) ||
		...
		if (read_config_option("auth_method") != AUTH_METHOD_NONE) {
			...
			$sql = "SELECT
				...
				FROM graph_tree
				LEFT JOIN user_auth_perms ON (graph_tree.id=user_auth_perms.item_id and user_auth_perms.type=" . PERM_TREES . " and user_auth_perms.user_id=" . $current_user["id"] . ")
				$sql_where

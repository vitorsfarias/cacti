...
function display_general() {
	...
	if ((file_exists(read_config_option("path_snmpget"))) && ((function_exists('is_executable')) && (is_executable(read_config_option("path_snmpget"))))) {
		$snmp_version = trim(shell_exec(escapeshellcmd(read_config_option("path_snmpget")) . " -V 2>&1"));
	}
...
function utilities_view_user_log() {
	...
	if (sizeof($user_log) > 0) {
		foreach ($user_log as $item) {
			...
			?>
			<td width='35%'>
				<?php print (strlen(get_request_var_request("filter")) ? (preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/i", "<span class=\"filter\">\\1</span>", $item["username"])) : $item["username"]);?>
			</td>
			<td width='20%'>
				<?php if (isset($item["full_name"])) {
						print (strlen(get_request_var_request("filter")) ? (preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/", "<span class=\"filter\">\\1</span>", $item["full_name"])) : $item["full_name"]);
					}else{
			...
			<td width='20%'>
				<?php if (isset($auth_realms[$item["realm"]])) {
						print (strlen(get_request_var_request("filter")) ? (preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/", "<span class=\"filter\">\\1</span>", $auth_realms[$item["realm"]])) : $auth_realms[$item["realm"]]);
					}else{
						print __("N/A");
					}
				?>
			...
			<td width='20%'>
				<?php print (strlen(get_request_var_request("filter")) ? (preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/i", "<span class=\"filter\">\\1</span>", $item["time"])) : $item["time"]);?>
			</td>
			<td width='10%'>
				<?php print $item["result"] == 0 ? __("Failed") : __("Success");?>
			</td>
			<td width='15%'>
				<?php print (strlen(get_request_var_request("filter")) ? (preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/i", "<span class=\"filter\">\\1</span>", $item["ip"])) : $item["ip"]);?>
			</td>
			<?php
			form_end_row();
		}

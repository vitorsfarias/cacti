...
function xaxis_item_edit() {
	...
	form_save_button_alt("path!xaxis.php|action!edit|id!" . get_request_var("xaxis_id"));
}
...
function xaxis_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		html_start_box(__("X-Axis Items"), "100", 0, "center", "xaxis.php?action=item_edit&xaxis_id=" . $_GET["id"], false, "xaxis");
		$header_items = array(
			array("name" => __("Item")),
			array("name" => __("Name")),
	...
	form_save_button("xaxis.php", "return");
        ...
        $('.delete').click(function (data) {
                ...
                request = "xaxis.php?action=item_remove_confirm&item_id="+id[0]+"&id="+id[1];
...
function xaxis_filter() {
	...
	html_start_box(__("X-Axis Presets"), "100", "3", "center", "xaxis.php?action=edit", true);
	?>

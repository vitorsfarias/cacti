...
if (sizeof($parms) == 0) {
	echo __("ERROR: You must supply valid input parameters!") . "\n";
	display_help($me);
	exit;
}
...
foreach($parms as $parameter) {
	...
	switch ($arg) {
		...
		case "-v":
		case "--version":
		case "--help":			display_help($me);				exit;
		default:
...
if (strtolower($device_id) == "all") {
	...
	$sql_where .= (strlen($sql_where) ? " AND " : " WHERE " ) . "data_local.device_id = '$device_id'";
}else{
	echo __("ERROR: You must supply either a device_id or 'All' to proceed.") . "\n";
	display_help($me);
	exit;
}
...
if (is_numeric($query_id)) {
	...
	echo __("ERROR: You must specify either a query_id or 'All' to proceed") . ".\n";
	display_help($me);
	exit;
}
...
echo __("WARNING: Do not interrupt this script.  Reordering can take quite some time") . "\n";
...
if (sizeof($data_queries)) {
	foreach ($data_queries as $data_query) {
		...
		update_data_query_sort_cache($data_query["device_id"], $data_query["snmp_query_id"]);
		/* build array required for function call */
		$data_query["snmp_index_on"] = get_best_data_query_index_type($data_query["device_id"], $data_query["snmp_query_id"]);
		/* as we request the output_type, "value" gives the snmp_query_graph_id */
		$data_query["snmp_query_graph_id"] = $data_query["value"]; 
		debug("Data Query #'" . $i . "' host: '" . $data_query["device_id"] .
...
function update_snmp_index_order($data_query) {
	if (is_array($data_query)) {
		...
		$snmp_cache_value = db_fetch_cell("SELECT field_value " .
				...
				"WHERE device_id='" . $data_query["device_id"] . "' " . 
...
function display_help ($me) {
	echo "Cacti Reorder Data Query Script 1.0" . ", " . __("Copyright 2004-2011 - The Cacti Group") . "\n";
	echo __("usage: reorder_data_query.php --id=[device_id|All] [--qid=[query_id]]") . "\n";
	echo __("usage: ") . $me . " --id=[device_id|All] [--qid=[query_id]]" . "\n";
	echo __("                           [-d] [-h] [--help] [-v] [--version]") . "\n\n";
	echo __("--id=device_id           - The device_id to have data queries reindexed; defaults to 'All' to reindex all hosts") . "\n";
	echo __("--qid=query_id           - Only index on a specific data query id") . "\n";

	...
	if (isset($_POST["selected_items"])) {
		...
		if (get_request_var_post("drp_action") == "1") { /* delete */
			for ($i=0;($i<count($selected_items));$i++) {
				...
				plugin_hook_function('user_remove', $selected_items[$i]);
			}
		}
...
function user_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		<table class='tabs' width='100%' cellspacing='0' cellpadding='3' align='center'>
			<tr>
				...
				<?php plugin_hook('user_admin_tab');?>				
				<td></td>
			</tr>
		</table>
		<?php
	...
	if (get_request_var_request("tab") == "graph_settings_edit") {
		...
		if (plugin_hook_function('user_admin_run_action', get_request_var_request("tab"))) {
			user_realms_edit();
		}
	}

...
function item_edit() {
	...
	$device = db_fetch_row("select hostname from host where id=" . get_request_var_request("host_id"));

	html_start_box2(__("Data Sources") . " " . __("[device: ") . (empty($device["hostname"]) ? __("No Host") : $device["hostname"]) . "]", "100", "3", "center", "");
						...
						<select name="cbo_host_id" onChange="window.location=document.form_graph_items.cbo_host_id.options[document.form_graph_items.cbo_host_id.selectedIndex].value">
							...
							if (sizeof($devices) > 0) {
								foreach ($devices as $device) {
									print "<option value='" . htmlspecialchars("graphs_items.php?action=item_edit" . $id . "&local_graph_id=" . get_request_var_request("local_graph_id") . "&host_id=" . $device["id"] . "&data_template_id=" . get_request_var_request("data_template_id")) . "'"; if (get_request_var_request("host_id") == $device["id"]) { print " selected"; } print ">" . $device["name"] . "</option>\n";
								}
							}
							?>
	...
	html_start_box2(__("Graph Items") . " $header_label", "100", 0, "center", "", true);
...
#	form_hidden_box("hidden_task_item_id", (isset($template_item) ? $template_item["task_item_id"] : "0"), "");
	...
	form_save_button("graphs.php?action=edit&id=" . get_request_var_request("local_graph_id"));

...
function createDQGraph($snmp_query_array, $graphTitle, $force) {
	...
	if (sizeof($snmp_indexes)) {
		...
		$devices = getDevices(array("id" => $snmp_query_array["host_id"]));
		echo __("ERROR: Could not find snmp-field %s (%d) for device-id %d (%s)", $snmp_query_array["snmp_field"], $snmp_query_array["snmp_value"], $snmp_query_array["host_id"], $devices[$snmp_query_array["host_id"]]["hostname"]) . "\n";
...
function display_help($me) {
	echo "Add Graphs Script 1.3" . ", " . __("Copyright 2004-2012 - The Cacti Group") . "\n";

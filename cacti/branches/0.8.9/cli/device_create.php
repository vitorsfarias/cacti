...
if (sizeof($parms)) {
	 ...
	 */
	if ($debug) {
		print("device_save(0, ".$device['site_id'].", ".$device['poller_id'].", ".$device['host_template_id'].", ".
		...
		$host_id = device_save(0, $device["host_template_id"],
		...
		$device_template["snmp_auth_protocol"], $device_template["snmp_priv_passphrase"],
		$device_template["snmp_priv_protocol"], $device_template["snmp_context"], $device_template["max_oids"], $device_template["device_threads"],
		$device["site_id"], $device["poller_id"]);
		...
		if (is_error_message()) {
			...
			echo __("Success - new device-id: (%d)", $host_id) . "\n";
			exit(0);
		}
	}
}else{
...
function display_help($me) {
	echo "Add Device Script 1.2" . ", " . __("Copyright 2004-2014 - The Cacti Group") . "\n";
...
#	echo "   --poller-id       1         " . __("the numerical ID of the poller") . "\n";
	...
	echo "   php -q " . $me . " --ip=example.company.com --description=foobar --template=1 --community=secret --avail=snmp --version=1\n";
	echo "   " . __("  same as above but overriding SNMP comunity with the value of 'secret', using SNMP V1") . "\n";

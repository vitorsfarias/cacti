...
#		}
		...
		if (!is_error_message()) {
			...
			if ($_POST["host_id"] != $_POST["hidden_host_id"]) {
				/* push out all necessary host information */
				push_out_host(get_request_var_post("host_id"), $local_data_id);

				/* reset current host for display purposes */
				$_SESSION["sess_data_source_currenthidden_host_id"] = $_POST["host_id"];
			}
	...
	if ((isset($_POST["save_component_data_source_new"])) && (empty($_POST["data_template_id"]))) {
		header("Location: " . html_get_location("data_sources.php") . "?action=edit&host_id=" . $_POST["host_id"] . "&new=1");
	}elseif ((is_error_message()) || ($_POST["data_template_id"] != $_POST["hidden_data_template_id"]) || ($_POST["data_input_id"] != $_POST["hidden_data_input_id"]) || ($_POST["host_id"] != $_POST["hidden_host_id"])) {
		header("Location: " . html_get_location("data_sources.php") . "?action=edit&id=" . (empty($local_data_id) ? $_POST["local_data_id"] : $local_data_id) . "&host_id=" . $_POST["host_id"] . "&view_rrd=" . (isset($_POST["current_rrd"]) ? $_POST["current_rrd"] : "0"));
	}else{
...
function data_source_form_actions() {
	...
	if (sizeof($ds_array)) {
		if (get_request_var_post("drp_action") === ACTION_NONE) { /* NONE */
			...
			print "	<tr>
					<td class='textArea'>
						...
						if (sizeof($graphs) > 0) {
							...
							print "<td class='textArea'><p class='textArea'>" . "The following Graph(s) are using these Data Source(s):" . "</p>\n";
			...
			$title = "Delete Data Source(s)";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_CHANGE_TEMPLATE) { /* change graph template */
			print "	<tr>
					<td class='textArea'>
						<p>" . "When you click 'Continue', the following Data Source(s) will be re-associated with the choosen Graph Template. Be aware that all warnings will be suppressed during the conversion, so Graph data loss is possible." . "</p>
						<div class='action_list'><ul>$ds_list</ul></div>
						<p><strong>". "New Data Source Template:" . "</strong><br>"; form_dropdown("data_template_id",db_fetch_assoc("select data_template.id,data_template.name from data_template order by data_template.name"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n";

			$title = "Change Data Source(s) Graph Template";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_CHANGE_HOST) { /* change host */
			print "	<tr>
					<td class='textArea'>
						<p>" . "When you click 'Continue', the following Data Source(s) will be re-associated with the Device below." . "</p>
						<div class='action_list'><ul>$ds_list</ul></div>
			...
			$title = "Change Data Source(s) Device";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_DUPLICATE) { /* duplicate */
			print "	<tr>
					<td class='textArea'>
						<p>" . "When you click 'Continue', the following Data Source(s) will be duplicated. You can optionally change the title format for the new Data Source(s)." . "</p>
						<div class='action_list'><ul>$ds_list</ul></div>
						<p><strong>" . "Title Format:" . "</strong><br>"; form_text_box("title_format", "<ds_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n";

			$title = "Duplicate Data Source(s)";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_CONVERT_TO_TEMPLATE) { /* data source -> data template */
			print "	<tr>
					<td class='textArea'>
						<p>" . "When you click 'Continue' the following Data Source(s) will be converted into Data Template(s).  You can optionally change the title format for the new Data Template(s)." . "</p>
						<div class='action_list'><ul>$ds_list</ul></div>
						<p><strong>" . "Title Format:" . "</strong><br>"; form_text_box("title_format", "<ds_title> Template", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n";

			$title = "Convert Data Soruce(s) to Data Template(s)";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_ENABLE) { /* data source enable */
			print "	<tr>
					<td class='textArea'>
			...
			$title = "Disable Data Source(s)";
		}elseif (get_request_var_post("drp_action") === DS_ACTION_REAPPLY_SUGGESTED_NAMES) { /* reapply suggested data source naming */
			print "	<tr>
					<td class='topBoxAlt'>
						<p>" . "When you click 'Continue', the following Data Source(s) will will have their suggested naming conventions recalculated." . "</p>
						<div class='action_list'><ul>$ds_list</ul></div>
					</td>
				</tr>\n";
...
function data_source_data_edit() {
	...
	if (!empty($data["data_input_id"])) {
		...
		html_start_box("Custom Data" . " " . "[data input:" . " " . db_fetch_cell("select name from data_input where id=" . $data["data_input_id"]) . "]", "100", "3", "center", "");
		...
		if (sizeof($fields) > 0) {
			foreach ($fields as $field) {
				...
				if ((!empty($host["id"])) && (preg_match('/^' . VALID_HOST_FIELDS . '$/i', $field["type_code"]))) {
					...
					print "<td width='50%'><strong>" . $field["name"] . "</strong> (" . "From Data Source Template" . ")</td>\n";
...
function data_source_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$().ready(function() {
			if ($("#hidden_data_template_id").val() == 0) {
				unlockTemplate();
				$("#data_source_options").closest("td").before("<td class='lock w1 textHeaderDark'><?php print "Template is unlocked";?></td>");
				disabled = false;
			}else{
				lockTemplate();
				$("#data_source_options").closest("td").before("<td class='lock w1 textHeaderDark'><?php print "Template is locked";?></td>");
				disabled = true;
			}
		});
	...
	if (!empty($_GET["host_id"]) || !empty($data_local["host_id"])) {
		$dd_menu_options .= '&host_id=' . (isset($_GET["host_id"]) ? $_GET["host_id"] : $data_local["host_id"]);
	}

	print "<form method='post' action='" .  basename($_SERVER["PHP_SELF"]) . "' name='data_source_edit'>\n";
	html_start_box("Data Source Template Selection" . " $header_label", "100", 0, "center", (isset($_GET["id"]) ? "menu::" . "Data Source Options" . ":data_source_options:html_start_box:" . $dd_menu_options : ""),"");
	...
	$form_array["host_id"]["id"] = (isset($_GET["host_id"]) ? $_GET["host_id"] : $data_local["host_id"]);
	$form_array["host_id"]["value"] = db_fetch_cell("SELECT CONCAT_WS('',description,' (',hostname,')') FROM host WHERE id=" . $form_array["host_id"]["id"]);
	...
	form_hidden_box("hidden_host_id", (empty($data_local["host_id"]) ? (isset($_GET["host_id"]) ? $_GET["host_id"] : "0") : $data_local["host_id"]), "");
	...
	if (!empty($data["data_template_id"])) {
		...
		html_start_box("Supplemental Data Source Template Data", "100", 0, "center", "");

		draw_nontemplated_fields_data_source($data["data_template_id"], $data["local_data_id"], $data, "|field|", "<strong>" . "Data Source Fields" . "</strong>", true, 0);
		draw_nontemplated_fields_data_source_item($data["data_template_id"], $data_source_items, "|field|_|id|", "<strong>" . "Data Source Item Fields" . "</strong>", true, true, 0);
		draw_nontemplated_fields_custom_data($data["id"], "value_|id|", "<strong>" . "Custom Data" . "</strong>", true, 0);
	...
	if (((isset($_GET["id"])) || (isset($_GET["new"]))) && (empty($data["data_template_id"]))) {
		...
		if (!empty($_GET["id"])) {
			...
			html_start_box("Data Source Items", "100", "0", "center", "data_sources_items.php?action=item_edit&local_data_id=" . $_GET["id"], true);
	...
	if ((isset($_SESSION["ds_debug_mode"])) && (isset($_GET["id"]))) {
		...
		<table width="100%" align="center">
			<tr>
				<td>
					<span class="textInfo"><?php print "Data Source Debug";?></span><br>
...
function data_source_filter() {
	...
	html_start_box("Data Sources", "100", "3", "center", "data_sources.php?action=edit&host_id=" . html_get_page_variable("host_id", true));
	?>
		...
		<td>
			...
			<table cellpadding="0" cellspacing="3">
				<tr>
					...
					<td class="w1">
						<input type="button" value="<?php print "Go";?>" name="go" align="middle" onClick="applyDSFilterChange(document.form_data_sources)">
					</td>
					<td class="w1">
						<input type="button" value="<?php print "Clear";?>" name="clear" align="middle" onClick="clearDSFilterChange(document.form_data_sources)">
					</td>
				</tr>
				<tr>
					<td class="w1">
					...
					<td class="w1">
						<?php
						if (html_get_page_variable("host_id") > 0) {
					...
					<td class="w1">
						<select name="method_id" onChange="applyDSFilterChange(document.form_data_sources)">
							<option value="-1"<?php if (html_get_page_variable("method_id") == "-1") {?> selected<?php }?>><?php print "Any";?></option>
							<option value="0"<?php if (html_get_page_variable("method_id") == "0") {?> selected<?php }?>><?php print "None";?></option>
							<?php
		...
		$("#host").autocomplete({
			...
			select: function(event, ui) {
				if (ui.item) {
					...
					// in case we didn't find anything, use "any" host
...
function get_data_source_records(&$total_rows, &$rowspp) {
	...
	if (html_get_page_variable("host_id") == "-1") {
		...
		$sql_where .= " AND data_local.host_id=" . html_get_page_variable("host_id");
	}

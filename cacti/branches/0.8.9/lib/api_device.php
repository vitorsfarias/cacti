...
function api_device_save($id, $host_template_id, $description, $hostname, $snmp_community, $snmp_version,
	...
	$save = plugin_hook_function('api_device_save', $save);
	...
	if (!is_error_message()) {
		...
		if (($host_template_id != $_host_template_id) && (!empty($host_template_id))) {
			...
			foreach ($graph_templates as $graph_template) {
				...
				plugin_hook_function('add_graph_template_to_host', array("host_id" => $host_id, "graph_template_id" => $graph_template["graph_template_id"]));
			}
			}
		}
	}
	...
	$save["id"] = $host_id;
	plugin_hook_function('api_device_new', $save);

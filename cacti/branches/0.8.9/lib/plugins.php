...
function do_hook ($name) {
	cacti_log("function " . __FUNCTION__ . " called using $name", false, "DEPRECATION WARNING");
	$data = func_get_args();
...
function api_plugin_register_hook ($plugin, $hook, $function, $file) {
	cacti_log("function " . __FUNCTION__ . " called using plugin $plugin, hook $hook", false, "DEPRECATION WARNING");
	plugin_register_hook($plugin, $hook, $function, $file);
}
...
function api_plugin_remove_hooks ($plugin) {
	cacti_log("function " . __FUNCTION__ . " called using plugin $plugin", false, "DEPRECATION WARNING");
...
function api_plugin_check_config ($plugin) {
	cacti_log("function " . __FUNCTION__ . " called using plugin $plugin", false, "DEPRECATION WARNING");
	return plugin_check_config($plugin);
}
/**
 * check a plugin
 * @param string $plugin	plugin name
...
function api_plugin_is_enabled ($plugin) {
	cacti_log("function " . __FUNCTION__ . " called using plugin $plugin", false, "DEPRECATION WARNING");
	return plugin_is_enabled($plugin);
}
/**
...
function api_user_realm_auth ($filename = '') {
	cacti_log("function " . __FUNCTION__ . " called using filename $filename", false, "DEPRECATION WARNING");
...
function plugin_upgrade_table($plugin, $table, $data, $sql_install_cache=false, $drop_items=false) {
	...
	if (in_array($table, $tables)) {
		...
		if (isset($data['columns']) && sizeof($data['columns'])) { 
			plugin_upgrade_columns($plugin, $table, $data['columns'], $sql_install_cache, $drop_items);
		}
		...
		if (isset($data['keys']) && sizeof($data['keys'])) { 
			plugin_upgrade_keys($plugin, $table, $data['keys'], $sql_install_cache, $drop_items);
		}
		
		// Check Engine ---------------------------------------------------------------------------------
		if (isset($data['engine'])) { # accept "engine"
			plugin_upgrade_engine($plugin, $table, $data['engine'], $sql_install_cache);
		}
		if (isset($data['type'])) { # ... as well as "type" (deprecated)
			plugin_upgrade_engine($plugin, $table, $data['type'], $sql_install_cache);
		}
		
		// Check auto_increment--------------------------------------------------------------------------------
		if (isset($data['auto_increment'])) { 
			plugin_upgrade_auto_increment($plugin, $table, $data['auto_increment'], $sql_install_cache);
		}
		
		// Check Comment ---------------------------------------------------------------------------------
		if (isset($data['comment'])) { 
			plugin_upgrade_comment($plugin, $table, $data['comment'], $sql_install_cache);
		}
		
		
	} else {
		// Table does not exist, so create it
		plugin_db_table_create($plugin, $table, $data, $sql_install_cache);
	}
}

/** DEPRECATED! Will vanish in 0.8.9++ */
function api_plugin_db_table_create ($plugin, $table, $data) {
	cacti_log("function " . __FUNCTION__ . " called using plugin $plugin for table $table", false, "DEPRECATION WARNING");
	plugin_db_table_create($plugin, $table, $data, false);
}
 ...
 * @param bool $sql_install_cache	- when using install_cache, the results will be presented to the user
 */
function plugin_upgrade_engine($plugin, $table, $engine, $sql_install_cache=false) {
	global $database_default;
...
/**
 * upgrade auto_increment of a given table
 * @param string $plugin			- name of the plugin/cacti version
 * @param string $table				- name of the table
 * @param string $auto_increment	- requested auto_increment for table
 * @param bool $sql_install_cache	- when using install_cache, the results will be presented to the user
 */
function plugin_upgrade_auto_increment($plugin, $table, $auto_increment, $sql_install_cache=false) {
	global $database_default;
	...
	/* upgrade in case of mismatch */
	if (isset($result['Auto_increment']) && strtolower($auto_increment) != strtolower($result['Auto_increment'])) {
		plugin_db_execute($table_sql . plugin_db_format_auto_increment_sql($auto_increment), $plugin, $sql_install_cache);
	}
...
/**
 * create sql to define the table auto_increment
 * @param string $auto_increment 	- the auto_increment for the table
 * @return string					- sql
 */
function plugin_db_format_auto_increment_sql($auto_increment) {
 ...
 * @param string $plugin			- name of the plugin/cacti version
 * @param string $table				- name of the table
 * @param string $comment			- requested comment for table
 * @param bool $sql_install_cache	- when using install_cache, the results will be presented to the user
 */
function plugin_upgrade_comment($plugin, $table, $comment, $sql_install_cache=false) {
	global $database_default;
...
/* -------------------------------------------------------------------------
 * functions to manage table keys 
   ---------------------------------------------------------------------- */
 ...
 */
function plugin_upgrade_keys($plugin, $table, $keys, $sql_install_cache=false, $drop_keys=false) {
	global $database_default;
...
/* -------------------------------------------------------------------------
 * functions to manage table columns 
   ---------------------------------------------------------------------- */
/**
 ...
 */
function plugin_upgrade_columns($plugin, $table, $columns, $sql_install_cache=false, $drop_columns=false) {
	global $database_default;
	...
	$result = db_fetch_assoc('SHOW FULL COLUMNS FROM `' . $table . '` FROM `' . $database_default . '` ') or die (mysql_error());
	...
	foreach ($columns as $column) {
		if (isset($column['name'])) {
			...
			if (isset($cols[$column['name']])) {
				...
				if (isset($column['collation']) && $column['collation'] != $c['Collation']) {
					$ok = FALSE;
				}

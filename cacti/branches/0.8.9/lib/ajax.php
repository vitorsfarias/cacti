...
function ajax_get_data_dd_menus() {
	...
	switch (get_request_var('cacti_dd_menu')) {
		...
		case 'device_options':
			...
			if (!empty($_GET["host_id"])) {
				$output =  "<h6><a href='" . htmlspecialchars('graphs.php?host_id=' . $_GET["host_id"] . '&template_id=-1&rows=-1&filter=') . "'>" . __("Graph Management") . "</a></h6>";
				$output .= "<h6><a href='" . htmlspecialchars('data_sources.php?host_id=' . $_GET["host_id"] . '&template_id=-1&rows=-1&method_id=-1&filter=') . "'>" . __("Data Source Management") . "</a></h6>";
				$output .= "<h6><a href='" . htmlspecialchars('graph_view.php?action=preview&host_id=' . $_GET["host_id"] . '&graph_template_id=0&filter=') . "'>" . __("Graph Preview") . "</a></h6>";
				$output .= "<h6><a href='" . htmlspecialchars('graph_view.php?action=list&host_id=' . $_GET["host_id"] . '&graph_template_id=0&filter=') . "'>" . __("Graph List View") . "</a></h6>";
			}
			break;
...
function ajax_get_devices_brief() {
	...
	$device_perms = db_fetch_cell("SELECT policy_hosts FROM user_auth WHERE id=" . $_SESSION["sess_user_id"]);
	 ...
	 * see $table->page_variables for the host_id "default" value */

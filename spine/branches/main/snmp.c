...
char *snmp_get(device_t *current_device, char *snmp_oid) {
	...
	if (current_device->snmp_session != NULL) {
		...
		if (!snmp_parse_oid(snmp_oid, anOID, &anOID_len)) {
			SPINE_LOG(("Host[%i] ERROR: SNMP Get Problems parsing SNMP OID %s", current_device->id, snmp_oid));
...
char *snmp_getnext(device_t *current_device, char *snmp_oid) {
	...
	if (current_device->snmp_session != NULL) {
		...
		if (!snmp_parse_oid(snmp_oid, anOID, &anOID_len)) {
			SPINE_LOG(("Host[%i] ERROR: SNMP Getnext Problems parsing SNMP OID %s", current_device->id, snmp_oid));
			SET_UNDEFINED(result_string);
			return result_string;
		}else{
			snmp_add_null_var(pdu, anOID, anOID_len);
...
int snmp_count(device_t *current_device, char *snmp_oid) {
	...
	if (current_device->snmp_session != NULL) {
		...
		if (!snmp_parse_oid(snmp_oid, root, &rootlen)) {
			SPINE_LOG(("Host[%i] ERROR: SNMP Count Problems parsing SNMP OID %s", current_device->id, snmp_oid));
			return count;
		}
